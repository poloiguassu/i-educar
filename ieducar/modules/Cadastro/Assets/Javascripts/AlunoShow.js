...
function fixupTabelaMatriculas() {
  ...
  if($j('#can_show_dependencia').val() == 1)
    $j('<th>').html(stringUtils.toUtf8('Depend̻ncia')).appendTo($tr);
    ...
    $j.each(dataResponse.matriculas, function(index, matricula) {
      ...
      if(matricula.user_can_change_situacao){
        var situacoes = [
              ...
              {val: 6, text: 'Abandono'},
              {val: 12, text: 'Aprovado com depend̻ncia'},
      ...
      if($j('#can_show_dependencia').val() == 1){
        var dependencia = matricula.dependencia == 't' ? 'Sim' : stringUtils.toUtf8('Ṇo');
