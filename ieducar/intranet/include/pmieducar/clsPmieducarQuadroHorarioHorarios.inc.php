...
class clsPmieducarQuadroHorarioHorarios
{
    ...
    public function __construct(
        ...
        $ativo = null,
        $dia_semana = null,
        $cod_quadro_horario_horarios = null,
        $data_aula = null
    ) {
        $db = new clsBanco();
        ...
        if (is_numeric($cod_quadro_horario_horarios)) {
            $this->cod_quadro_horario_horarios = $cod_quadro_horario_horarios;
        }
    ...
    public function lista(
        ...
        $int_cod_quadro_horario_horarios    = null
    ) {
        ...
        if (is_numeric($int_cod_quadro_horario_horarios)) {
            $filtros .= "{$whereAnd} qhh.cod_quadro_horario_horarios = '{$int_cod_quadro_horario_horarios}'";
            $whereAnd = ' AND ';
        }
                    ...
                    AND
                        qh.ano = 
                            (
                                ...
                                WHERE
                                    eal.ref_cod_escola = qhh.ref_cod_escola
                            )
                )";
        ...
        if (is_numeric($this->ref_cod_quadro_horario)
            ...
            $db->Consulta(
                ...
                AND
                    ref_cod_escola = '{$this->ref_ref_cod_escola}'
                AND
                    ref_cod_disciplina = '{$this->ref_ref_cod_disciplina}'
                AND
            ...
            return $db->Tupla();
        } elseif (is_numeric($ref_cod_escola)
            && is_numeric($this->ref_cod_instituicao_servidor) 
            ...
            $db = new clsBanco();
            $db->Consulta(
                "SELECT {$this->_todos_campos}
                FROM
                    {$this->_tabela}
                WHERE
                    ref_cod_escola = {$ref_cod_escola}
                AND
                    ref_cod_instituicao_servidor = {$this->ref_cod_instituicao_servidor}
                AND
            ...
            return $db->Tupla();
        } else if (is_numeric($this->cod_quadro_horario_horarios)) {
            $db = new clsBanco();
            $db->Consulta(
                "SELECT {$this->_todos_campos}
                FROM
                    {$this->_tabela}
                WHERE
                    cod_quadro_horario_horarios = {$this->cod_quadro_horario_horarios}
                AND
                    ativo = 1"
            );
            $db->ProximoRegistro();
    ...
    public function existe()
    {
        if (is_numeric($this->ref_cod_quadro_horario)
            ...
            if (is_numeric($this->cod_quadro_horario_horarios)) {
                $db->Consulta(
                    "SELECT
                        1
                    FROM
    ...
    public function conta(
        ...
        $int_ref_ref_cod_escola             = null,
        $int_ref_ref_cod_disciplina         = null,
        $time_data_aula_ini                 = null,
        $time_data_aula_fim                 = null,
        $int_ref_cod_instituicao_servidor   = null,
        $int_ref_servidor                   = null,
        $time_hora_inicial_ini              = null,
        $time_hora_inicial_fim              = null,
        ...
        if ($unico) {
            $distinct = "DISTINCT ON (qhh.data_aula, qhh.ref_cod_quadro_horario)";
        }

        $sql = "SELECT {$distinct} cod_quadro_horario_horarios FROM {$this->_tabela} qhh";
        $filtros = '';
        ...
        if (is_string($time_data_aula_ini)) {
            $filtros .= "{$whereAnd} qhh.data_aula >= '{$time_data_aula_ini}'";
            $whereAnd = ' AND ';
        }

        if (is_string($time_data_aula_fim)) {
            $filtros .= "{$whereAnd} qhh.data_aula <= '{$time_data_aula_fim}'";
            $whereAnd = ' AND ';
        }
        ...
        if (is_numeric($int_filtrar_turma)) {
            $filtros .= "{$whereAnd}
                EXISTS
                (
                    SELECT 
                    ...
                    AND 
                        ...
                        (
                            SELECT min(cod_quadro_horario) FROM pmieducar.quadro_horario WHERE ref_cod_turma = 6
                        )
                )";
        ...
        return $db->CampoUnico("SELECT COUNT(0) FROM ({$sql}) as temp");
    }
    ...
    public function retornaHorario(
        ...
        if (is_numeric($int_ref_cod_instituicao_servidor)
            ...
            && is_numeric($int_ref_ref_cod_turma)
        ) {
            $db = new clsBanco();
            $db->Consulta(
                "SELECT
                ...
                WHERE
                    ...
                    cc.id = qhh.ref_cod_disciplina AND
                    qhh.ativo = 1
                ORDER BY
                    data_aula,
                    hora_inicial ASC"
            ...
            while ($db->ProximoRegistro()) {
                ...
                $tupla['_total'] = $this->_total;
                $resultado[$tupla['data_aula']][] = $tupla;
            }
