<table summary="" class='tabelanum1' border='0' cellspacing='0' cellpadding='0'>
<tr class="r2c1">
	<td align="left" width="40%" class="r2c1A" >&nbsp;Usuário atual: <b><!-- #&USERLOGADO&# --></b></td>
	<td align="left" width="20%" class="r2c1A" >&nbsp;Último Acesso: <b><!-- #&ULTIMOACESSO&# --></b></td>
	
	<td align="right" width="40%"><a href="meusdados.php"><img src="imagens/nvp_bot_intra_dados.jpg" border="0" alt="Meus Dados" title="Meus Dados"></a><a href="logof.php"><img src="imagens/nvp_bot_intra_logout.jpg" border="0" alt="Logout" title="Fazer Logout"></a><a href="http://www.cobra.com.br/"><img src="imagens/nvp_bot_internet.jpg" border="0" alt="Acessar Internet" title="Acessar PÃ¡gina na Internet"></a></td>
</tr>
<tr>
	<td colspan="3">
	<table summary="" class='tabelanum2' border='0' cellspacing='0' cellpadding='0'>
	<tr>
		<td valign=top><!-- #&CORPO&# --></td>
	</tr>
	</table>
	</td>
</tr>
<tr class="rodape">
	<td colspan="3"><center>
		<img src="imagens/nvp_bot_html.jpg" border="0" alt="HTML Validate" title="HTML Validate">
		<img src="imagens/nvp_bot_css.jpg" border="0" alt="CSS Validate" title="CSS Validate">
		<img src="imagens/nvp_bot_xml.jpg" border="0" alt="XML Validate" title="XML Validate">
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<img src="imagens/nvp_bot_acessibilidade.jpg" border="0" alt="Este sí­mbolo indica o ato voluntário que demonstra, unicamente, um esforÃ§o em aumentar a acessibilidade de um site." title="Este símbolo indica o ato voluntário que demonstra, unicamente, um esforço em aumentar a acessibilidade de um site.">
	</center></td>
</tr>
</table>
